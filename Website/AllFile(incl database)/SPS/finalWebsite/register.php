...
<!-- Profile Session -->				
				<div id="pagebodysidediv">
				<div id="alignRight">
<a href="editProfile.html">Edit </a>
</div>
				<div id="idPic"> <img src="./css/blank-face.jpg" alt="profilePic" style="width:100;height:120px;"></img> </div>
				<br/>
				
				<TABLE  CLASS="datadisplaytable" >
<?php

	//modify id to get data
	...
	while ($row = mysqli_fetch_row($result)){
			if ($row[0]==$id){
				...
				$_SESSION['postcode']=$row[6];
				break;
			}
	}

	echo "<TR>";
	echo "<TH COLSPAN=\"2\" CLASS=\"ddlabel\" scope=\"row\" >Full Name:</TH>";
	echo "<TD CLASS=\"dddefault\">".$_SESSION['fName']." ".$_SESSION['lName']."</TD>"; 
?>
</TR>
<TR>
<TH COLSPAN="2" CLASS="ddlabel" scope="row" >Phone: </TH>
<?php echo "<TD CLASS=\"dddefault\">".$_SESSION['phone']."</TD>"; ?>
</TR>
<TR>
<TH COLSPAN="2" CLASS="ddlabel" scope="row" >Email: </TH>
<?php echo "<TD CLASS=\"dddefault\">".$_SESSION['email']."</TD>"; ?>
</TR>

<TR>
<TH COLSPAN="2" CLASS="ddlabel" scope="row" >Address</TH>
<?php echo "<TD CLASS=\"dddefault\">".$_SESSION['road']."<br/>".$_SESSION['state']."<br/>".$_SESSION['postcode']."</TD>"; ?>
</TR>
<TR>
</table>
				</div>
...
<?php
	...
	while ($row = mysqli_fetch_row($result1)){
		if (($row[0]==$id)&&($row[3]==0)){
			echo "<h4>Study plan for: " . $row[2] ."</h4>";
			$_SESSION['studyPlan'] = $row[2];
			echo "<br/>";
		...
		echo "</table>";
		$getData=true;
		$_SESSION['crnList'] = $crnList;
		break;
		}
	}
	
	if (!($getData)) {
			echo "<h4>You haven't enrolled any subject. Click <a href=\"#\">here</a> to enroll for new semester</h4>";
		}
	//select the Result from database
